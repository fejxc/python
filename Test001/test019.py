...
def get_centenses_from_file(filename):
    with open(filename) as file:
        text = file.read()  # read str from file
    lines=text.split('.')
    return  lines
...
def main():
    ...
    sentence_count= get_centenses_from_file(filename)


    # display number of words and sentences and unique word
    print("Number of sentences = ",len(sentence_count))
